...
from JigClu.loader import GaussianBlur, JigCluTransform, Solarize

from JigClu import resnet_jigclu as models
from JigClu.loader import GaussianBlur,Solarize, JigCluTransform
from JigClu.jigclu import JigClu
...
parser.add_argument('--loss_info', default=0.5, type=float,
                    help='loss weight for location branch')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = datasets.ImageFolder(
        traindir,
        JigCluTransform(transforms.Compose(augmentation),transforms.Compose(aug_info), args.cross_ratio))
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        permute = torch.randperm(n*4)
        images = torch.cat(images, dim=0)
        images = images[permute,:,:,:]
        col1 = torch.cat([images[0:n], images[n:2*n]], dim=3)
        col2 = torch.cat([images[2*n:3*n], images[3*n:]], dim=3)
        images = torch.cat([col1, col2], dim=2)
